...
if [[ "$USE_ZH" == "true" ]]; then
 ...
 #   --app app_name_2 \\                    #
 #   --1panel-path /opt                    #"
    if [[ "$USE_ZH_IS_NOT_CONFIGURED" == "true" ]]; then
        MSG_INTRO+="
 #                                         #
 # 当前根据你的服务器IP显示为中文, 配置环境#
 # 变量 USE_ZH(true|false) 手动选择显示中文#
 # or English                              #
 ###########################################"
    else
        MSG_INTRO+="
 ...
 #   --1panel-path /opt                    #"
    if [[ "$USE_ZH_IS_NOT_CONFIGURED" == "true" ]]; then
        MSG_INTRO+="
 #                                         #
 # Currently displayed in the language     #
 # detected from your server IP. Set the   #
 # USE_ZH (true|false) env var to manually #
